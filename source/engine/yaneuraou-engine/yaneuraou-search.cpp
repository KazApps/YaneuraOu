...
int correction_value(const YaneuraOuWorker& w, const Position& pos, const Stack* const ss) {
    ...
    return 11160 * pcv + 8820 * micv + 6656 * (wnpcv + bnpcv) + 8222 * cntcv;
}
...
void update_correction_history(const Position&          pos,
    ...
    constexpr int nonPawnWeight = 205;

    workerThread.pawnCorrectionHistory[pawn_correction_history_index(pos)][us] << bonus;
    workerThread.minorPieceCorrectionHistory[minor_piece_index(pos)][us] << bonus * 137 / 128;
    ...
    if (m.is_ok())
    {
        ...
        (*(ss - 2)->continuationCorrectionHistory)[pc][to] << bonus * 222 / 128;
        (*(ss - 4)->continuationCorrectionHistory)[pc][to] << bonus * 83 / 128;
    }
}
...
void YaneuraOuWorker::clear() {
    ...
    mainHistory.fill(56);
    captureHistory.fill(-668);
    pawnHistory.fill(-1373);
    pawnCorrectionHistory.fill(-10);
    minorPieceCorrectionHistory.fill(-3);
    ...
    for (size_t i = 1; i < reductions.size(); ++i)
        reductions[i] = int(2733 / 128.0 * std::log(i));
...
Value YaneuraOuWorker::search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode)
{
	...
	*/
    ...
    {
        int evalDiff = std::clamp(-int((ss - 1)->staticEval + ss->staticEval), -188, 168) + 71;
	...
	*/
    ...
    if (priorReduction >= 3 && !opponentWorsening)
        depth++;
    if (priorReduction >= 2 && depth >= 2 && ss->staticEval + (ss - 1)->staticEval > 257)
        depth--;
    ...
    if (!PvNode && eval < alpha - 613 - 189 * depth * depth)
	...
	*/

    {
		...
		auto futility_margin = [&](Depth d) {
            Value futilityMult = 107 - 13 * !ss->ttHit;

            return futilityMult * d                                //
                 - 2380 * improving * futilityMult / 1024          //
                 - 1918 * opponentWorsening * futilityMult / 4096  //
                 + std::abs(correctionValue) / 123342;
        };
    ...
    if (cutNode && ss->staticEval >= beta - 23 * depth + 327 && !excludedMove
#if STOCKFISH
	...
	// probCutに使うbeta値。
	probCutBeta = beta + 339 - 62 * improving;
        ...
        && !(is_valid(ttData.value) && ttData.value < probCutBeta))
    {
		...
		Depth      probCutDepth = std::clamp(depth - 5 - (ss->staticEval - beta) / 366, 0, depth);
...
// 🤔 Stockfish本家のこの読み筋の出力、細かすぎるので時間をロスする。しないほうがいいと思う。
            ...
            && !is_loss(bestValue))
        {
            ...
            if (capture || givesCheck)
            {
                ...
                if (!givesCheck && lmrDepth < 7)
                {
					...
					Value futilityValue = ss->staticEval + 210 + 288 * lmrDepth
										+ PieceValue[capturedPiece] + 110 * captHist / 1024;
                ...
                int margin = std::max(192 * depth + captHist / 25, 0);
#if STOCKFISH
            ...
            else
            {
                ...
                if (history < -4354 * depth)
                    continue;

                history += 90 * mainHistory[us][move.raw()] / 32;

				// (*Scaler): Generally, lower divisors scales well
				// 一般に、割る数（divisor）が小さいほどスケールしやすい。

				lmrDepth += history / 3682;

				Value futilityValue = ss->staticEval + 51 + 268 * !bestMove + 122 * lmrDepth
                    + 82 * (ss->staticEval > alpha);
		...
		*/
            ...
            && ttData.depth >= depth - 3)
        {
			...
			*/
            ...
            Value singularBeta  = ttData.value - (43 + 74 * (ss->ttPv && !PvNode)) * depth / 64;
            ...
            {
                int corrValAdj   = std::abs(correctionValue) / 168245;
                int doubleMargin = -5 + 175 * PvNode - 174 * !ttCapture - corrValAdj
                                 - 990 * ttMoveHistory / 147721 - (ss->ply > rootDepth) * 36;
                int tripleMargin = 49 + 418 * PvNode - 229 * !ttCapture + 104 * ss->ttPv - corrValAdj
                                 - (ss->ply * 2 > rootDepth * 3) * 49;
			...
			*/
            ...
            {
                ttMoveHistory << std::max(-362 - 116 * depth, -5871);
                return value;
            }
		...
		if (ss->ttPv)
            r -= 3251 + PvNode * 417 + (ttData.value > alpha) * 1092
               + (ttData.depth >= depth) * (1740 + cutNode * 1295);

        // These reduction adjustments have no proven non-linear scaling
        // これらの減少量調整には、非線形スケーリングの有効性が証明されていません

        r += 1241;  // Base reduction offset to compensate for other tweaks
				   // 他の調整を補正するための基準リダクションオフセット

		r -= moveCount * 38;
        r -= std::abs(correctionValue) / 16240;
		...
		*/
        ...
        if (capture)
            ss->statScore = 954 * int(PieceValue[pos.captured_piece()]) / 128
		...
		*/
        ...
        if (depth >= 2 && moveCount > 1)
        {
			...
			*/
            ...
            if (value > alpha)
            {
                ...
                const bool doDeeperSearch = d < newDepth && value > (bestValue + 30 + 2 * newDepth);
                const bool doShallowerSearch = value < bestValue + 7;
            ...
            else if (value > alpha && value < bestValue + 11)
                newDepth--;
        }
        ...
        else if (!PvNode || moveCount > 1)
        {
			...
			value = -search<NonPV>(pos, ss + 1, -(alpha + 1), -alpha,
                                   newDepth - (r > 3048) - (r > 3595 && newDepth > 2), !cutNode);
		}
	...
	else if (bestMove)
    {
        ...
        if (!PvNode)
            ttMoveHistory << (bestMove == ttData.move ? 1202 : -739);
    }
	...
	*/
    ...
    {
        int bonusScale = -250;
        bonusScale -= (ss - 1)->statScore / 127;
        bonusScale += std::min(96 * depth, 539);
        bonusScale += 203 * ((ss - 1)->moveCount > 8);
        bonusScale += 108 * (!ss->inCheck && bestValue <= ss->staticEval - 83);
        bonusScale += 94 * (!(ss - 1)->inCheck && bestValue <= -(ss - 1)->staticEval - 77);

        bonusScale = std::max(bonusScale, 0);

        const int scaledBonus = std::min(185 * depth - 71, 1658) * bonusScale;

        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq,
                                      scaledBonus * 364 / 32768);
	...
	*/
    ...
    else
    {
		...
		*/ 
		...
		futilityBase = ss->staticEval + 434;
...
Depth Search::YaneuraOuWorker::reduction(bool i, Depth d, int mn, int delta) const {
    ...
    return reductionScale - delta * 1149 / rootDelta + !i * reductionScale * 137 / 512 + 1088;
}
...
void update_all_stats(const Position&          pos,
	...
	int bonus = std::min(98 * depth - 80, 882) + 416 * (bestMove == ttMove);
	int malus = std::min(899 * depth - 206, 2890) - 9 * moveCount;
	...
	*/
    ...
    {
        update_quiet_histories(pos, ss, workerThread, bestMove, bonus * 1097 / 1024);

        // Decrease stats for all non-best quiet moves
        // 最善でないquietの指し手すべての統計を減少させる

        for (Move move : quietsSearched)
            update_quiet_histories(pos, ss, workerThread, move, -malus * 1362 / 1024);
    }
    else
    {
        ...
        captureHistory[movedPiece][bestMove.to_sq()][capturedPiece] << bonus * 1539 / 1024;
    }
    ...
    if (prevSq != SQ_NONE && ((ss - 1)->moveCount == 1 + (ss - 1)->ttHit) && !pos.captured_piece())
        update_continuation_histories(ss - 1, pos.piece_on(prevSq), prevSq, -malus * 663 / 1024);
    ...
    for (Move move : capturesSearched)
    {
		...
		*/
        ...
        captureHistory[movedPiece][move.to_sq()][capturedPiece] << -malus * 1232 / 1024;
    }
}
...
void update_continuation_histories(Stack* ss, Piece pc, Square to, int bonus) {
    static constexpr std::array<ConthistBonus, 6> conthist_bonuses = {
      {{1, 1352}, {2, 635}, {3, 337}, {4, 696}, {5, 173}, {6, 371}}};
    ...
    for (const auto [i, weight] : conthist_bonuses)
    {
        ...
        if (((ss - i)->currentMove).is_ok())
            (*(ss - i)->continuationHistory)[pc][to] << (bonus * weight / 1024) + 133 * (i < 2);
    }
}
...
void update_quiet_histories(
  const Position& pos, Stack* ss, Search::YaneuraOuWorker& workerThread, Move move, int bonus) {
    ...
    if (ss->ply < LOW_PLY_HISTORY_SIZE)
        workerThread.lowPlyHistory[ss->ply][move.raw()] << bonus * 582 / 1024;

    update_continuation_histories(ss, pos.moved_piece(move), move.to_sq(), bonus * 740 / 1024);

    int pIndex = pawn_history_index(pos);
    workerThread.pawnHistory[pIndex][pos.moved_piece(move)][move.to_sq()]
      << bonus * (bonus > 0 ? 1007 : 642) / 1024;
}
