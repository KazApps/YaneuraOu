...
void Position::set(std::string sfen , StateInfo* si , Thread* th)
{
	...
	while ((ss >> token) && !isspace(token))
	{
		...
		else if ((idx = PieceToCharBW.find(token)) != string::npos)
		{
			...
			ct = max(ct, 1);
			add_to_hand(color_of(Piece(idx)), type_of(Piece(idx)), ct);
...
void Position::do_move_impl(Move m, StateInfo& new_st, bool givesCheck)
{
	...
	st->accumulator.computed[BLACK] = st->accumulator.computed[WHITE] = false;
#endif
	...
	if (m.is_drop())
	{
		...
		put_piece(to, pc);
		[[maybe_unused]] int bit = sub_from_hand<Us>(pr);

		// 打駒した駒に関するdirtPieceの更新。
#if defined (EVAL_NNUE)
	  ...
	  // 移動先で駒を捕獲するときの利きの更新
			...
			// 駒取りなら現在の手番側の駒が増える。
			[[maybe_unused]] int bit = add_to_hand<Us>(pr);
	
			// 捕獲される駒の除去
			remove_piece(to);

			// 捕獲した駒に関するDirtyPieceの更新
#if defined (EVAL_NNUE)
...
#if defined (EVAL_NNUE)
		dp.piece[0] = piece_on(from);
		dp.from[0] = from;
		dp.to[0] = to;
		dp.promote = m.is_promote();
#endif
...
void Position::undo_move_impl(Move m)
{
	...
	if (m.is_drop())
	{
		...
		if (st->capturedPiece != NO_PIECE)
		{
			...
			// 手駒から減らす
			sub_from_hand<Us>(raw_type_of(to_pc));
